-- ============================================
-- MIGRATION - Separar Hierarquia de Função (CORRIGIDA)
...
-- 1. Adicionar campo 'hierarchy' (hierarquia funcional) - NULLABLE primeiro
...
UPDATE resources
SET hierarchy = CASE
  WHEN role = 'gerente' THEN 'gerente'
  WHEN role = 'lider' THEN 'lider'
  WHEN role = 'operador' THEN 'operador'
  ELSE NULL
END
WHERE role IN ('gerente', 'lider', 'operador');

-- 3. Definir hierarquia padrão para recursos sem hierarquia
-- Todos os recursos devem ter uma hierarquia (padrão: operador)
UPDATE resources
SET hierarchy = 'operador'
WHERE hierarchy IS NULL;

-- 4. Limpar o campo 'role' onde era hierarquia (não especialidade)
-- Isso deixa role NULL para que o usuário possa adicionar a especialidade depois
UPDATE resources
SET role = NULL
WHERE role IN ('gerente', 'lider', 'operador');

-- 5. Tornar hierarchy obrigatório DEPOIS de preencher todos
ALTER TABLE resources
